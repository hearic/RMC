...
void CDInput::SI_ReadConvergenceBlock(CDConvergence &cConvergence,int nINeuNumPerCyc)
{
	////////////////////// avoid redefinition ////////////////////////
	...
	for(;;)
	{
		...
		if(InputCardStr.compare("SEMESH")==0)  
		{
			...
			SI_ReadMeshPara(cConvergence.p_OShanEntropy.p_OSeMesh,"Shannon Entropy",bIsBlockEnd);
		...
		else if(InputCardStr.compare("FMMESH")==0)   //// allow multiple definition of fission matrix
		{
			...
			SI_ReadMeshPara(cConvergence.p_vDiagFisMatrix[size].p_OFmMesh,"Fission Matrix",bIsBlockEnd);
		...
		else if(InputCardStr.compare("NPMMESH")==0)  // calculate dominance ratio
		{
			...
			SI_ReadMeshPara(cConvergence.p_vNpMatrix[size].p_ONpMesh,"Noise Propagation Matrix",bIsBlockEnd);
