...
void CDInput::SI_ReadCellSurfBool(int nCell_index,bool &bIsBlockEnd,bool &bIsCardEnd,CDGeometry &cGeometry)
{
	bIsBlockEnd = false;
	bIsCardEnd = false;
	...
	for(;;)
	{
		...
		if(nextchar == '$'||nextchar == '%'||nNext_p==4) //current universe or cell finished
		{
