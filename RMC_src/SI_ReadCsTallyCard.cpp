...
void CDInput::SI_ReadCsTallyCard(bool &bIsBlockEnd,vector<CDCsTally> &vOneGroupCsTally,bool &bIsWithCsTally,CDGeometry &cGeometry,CDMaterial &cMaterial)
{
	bIsBlockEnd = false;
	...
	for(;;)
	{
		...
		{
			if(nextchar == '%'||nNext_p == 4)   { bIsBlockEnd = true; }
			break;
		} 
		...
		if(KeyWordStr.compare("CELL") == 0)  //// Read Tally Cell 
		{
			...
			Output.CheckInputParas(SI_ReadTallyCells(vTallyCells,bIsBlockEnd,bIsCardEnd,cGeometry) && vTallyCells.size() == 1,
		...
		else if(KeyWordStr.compare("MT") == 0)  //// Read Tally MT arrays 
		{
			...
			{
				vector<int> vMTs;
				int nNext_p = CDFileIO::SI_ReadVaryVec(p_fpInputFilePtr,vMTs,"%d");
