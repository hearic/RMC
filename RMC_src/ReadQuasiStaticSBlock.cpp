...
void CDInput::ReadQuasiStaticSBlock(CDCriticality &cC_Criticality)
{

	cC_Criticality.p_bIsQuasiStaticS = true;
	...
	for(;;)
	{
		...
		if(InputCardStr.compare("POWERITER")==0)  
		{
			...
			Output.CheckInputParas(!DefinedCard[InputCardStr],
				[&](){sprintf(Output.p_sPrintStr,"%s card is redefined in QUASISTATIC_S block.\n",InputCardStr.c_str());},2);
			...
			if(vParas[2].size() > 0)
			{
				...
				Output.CheckInputParas(cC_Criticality.p_nBatchNum > 0 && cC_Criticality.p_nActCycNum >= cC_Criticality.p_nBatchNum && cC_Criticality.p_nActCycNum % cC_Criticality.p_nBatchNum == 0,
					[&](){sprintf(Output.p_sPrintStr,"incorrect BATCH parameters in POWERITER card. BatchNum = %d\n",cC_Criticality.p_nBatchNum);},CDOutput::_ERROR);
		...
		else if(InputCardStr.compare("INITSRC")==0)  
		{
			...
			ReadCardOptions(string("InitSrc"),vOptions,vParaNum,vParas,bIsBlockEnd);
		...
		else if(InputCardStr.compare("TIMESTEP")==0) 
		{
			...
			vector<int> vParaNum(1);
			vector<vector<double> > vParas;
			vOptions[0] = "DELTAT" ;    vParaNum[0] = 1 ;
			ReadCardOptions(InputCardStr,vOptions,vParaNum,vParas,bIsBlockEnd);
			if(vParas[0].size() > 0)
			{
				cC_Criticality.p_dDeltaTimeNext = vParas[0][0]*1e+8; //unit: shake = 1e-8 sec
				Output.CheckInputParas(cC_Criticality.p_dDeltaTimeNext > 0,
					[&](){sprintf(Output.p_sPrintStr,"Wrong time step in TIMESTEP card.\n");},2);
			}
		...
		{
			Output.OutputMessage([&](){sprintf(Output.p_sPrintStr,"unknown input card %s in QUASISTATIC_S block.\n",InputCardStr.c_str());}, 
				Output.p_fpOutputFilePtr, CDOutput::_ERROR);
		}
	}
