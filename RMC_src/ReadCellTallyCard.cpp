...
void CDInput::ReadCellTallyCard(bool &bIsBlockEnd,vector<CDCellTally> &vCellTally,bool &bIsWithCellTally,CDGeometry &cGeometry)
{
...
//	vector<CDCellTally>  &vCellTally = vCellTally;
	...
	for(;;)
	{
		...
		else if(KeyWordStr.compare("ENERGY") == 0) //// Read energy bins 
		{
			...
			///////////// (-,e0),(e0,e1), (e1,e2),...,(en,+) ////////////////////
			CDFileIO::SkipSpecChar(p_fpInputFilePtr,'=');
			int nNext_p = CDFileIO::ReadVaryVec(p_fpInputFilePtr,vCellTally[nTallySize].p_vErgBins,"%lf");
			if(nNext_p >= 2)   
			{
				bIsCardEnd = true;
		...
		else if (KeyWordStr.compare("FILTER")==0) //// Read tally filter
		{
			...
			CDFileIO::SkipSpecChar(p_fpInputFilePtr,'=');
			int nNext_p = CDFileIO::ReadVaryVec(p_fpInputFilePtr,vCellTally[nTallySize].p_vFilter,"%d");
			if(nNext_p >= 2)   
			{
				bIsCardEnd = true;
				if(nNext_p >= 3) { bIsBlockEnd = true;}
		...
		else if (KeyWordStr.compare("INTEGRAL")==0)  //// Read integral tally
		{
			...
			CDFileIO::SkipSpecChar(p_fpInputFilePtr,'=');
			int nNext_p = CDFileIO::ReadVaryVec(p_fpInputFilePtr,vCellTally[nTallySize].p_vIntgrlBins,"%d");
		...
		else if(KeyWordStr.compare("CELL") == 0)  //// Read Tally Cells 
		{
			...
			Output.CheckInputParas(ReadTallyCells(vTallyCells,bIsBlockEnd,bIsCardEnd,cGeometry),// read tally cells
