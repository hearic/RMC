...
void CDInput::ReadTallyBlock(CDTally &cTally,CDGeometry &cGeometry,CDMaterial &cMaterial)
{
	...
	for(;;)
	{
		...
		{
			ReadCellTallyCard(bIsBlockEnd,cTally.p_vCellTally,cTally.p_bIsWithCellTally,cGeometry);
		}
		else if(InputCardStr.compare("MESHTALLY")==0)  
		{
			ReadMeshTallyCard(bIsBlockEnd,cTally.p_vMeshTally,cTally.p_bIsWithMeshTally );
		}
		else if(InputCardStr.compare("CSTALLY")==0)  
		{
			ReadCsTallyCard(bIsBlockEnd,cTally.p_vOneGroupCsTally,cTally.p_bIsWithCsTally,cGeometry,cMaterial);
		}
