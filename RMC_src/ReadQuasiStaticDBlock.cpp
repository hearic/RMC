...
void CDInput::ReadQuasiStaticDBlock(CDCriticality &cC_Criticality)
{

	cC_Criticality.p_bIsQuasiStaticD = true;
	...
	for(;;)
	{
		...
		if(InputCardStr.compare("POWERITER")==0)  
		{
			...
			Output.CheckInputParas(!DefinedCard[InputCardStr],
				[&](){sprintf(Output.p_sPrintStr,"%s card is redefined in QUASISTATIC_D block.\n",InputCardStr.c_str());},2);
...
//		}
		...
		{
			Output.OutputMessage([&](){sprintf(Output.p_sPrintStr,"unknown input card %s in QUASISTATIC_D block.\n",InputCardStr.c_str());}, 
				Output.p_fpOutputFilePtr, CDOutput::_ERROR);
		}
	}
